...
export function watchSignal(parent: Context, signal: AbortSignal): Context {
	...
	const onAbort = () => {
		context.cancel(
			(signal as any).reason instanceof Error ? ((signal as any).reason as Error) : ContextCancelledError
		);
	};
...
export function withAbort(parent: Context): [Context, AbortController] {
	...
	const onAbort = () => {
		context.cancel(
			(ac.signal as any).reason instanceof Error ? ((ac.signal as any).reason as Error) : ContextCancelledError
		);
	};
