...
export async function select<T = any>(cases: SelectCase<T>[]): Promise<void> {
	...
	if (defaultCase) {
		const hasReadyOperation =
			receiveCases.some((case_) => case_.channel.hasData()) || sendCases.some((case_) => case_.channel.canSend());
	...
	const promises: Promise<{
		type: "receive" | "send";
		case_: ReceiveCase<T> | SendCase<T>;
		value?: T;
		ok?: boolean;
	}>[] = [];
	...
	for (const case_ of receiveCases) {
		promises.push(case_.channel.receive().then(([value, ok]) => ({ type: "receive" as const, case_, value, ok })));
	}

	// Add send promises
	for (const case_ of sendCases) {
		promises.push(case_.channel.send(case_.value).then(() => ({ type: "send" as const, case_ })));
	}
